function [ x_best, y_best, rerr, it_cnt, fe_cnt ] = Nav_nlm_2d_solve_demo(eps, base_elements, x_prev, y_prev, z,...
...
while ~is_finished 
    ...
    for n = 1:3
        fxi(n) = eps(base_elements, xix(n), xiy(n), z);
        fe_cnt = fe_cnt + 1;
        plot(xix(n), xiy(n), '.r');
    end            
    line(xix, xiy, 'color', 'red');
    pause(0.1);
