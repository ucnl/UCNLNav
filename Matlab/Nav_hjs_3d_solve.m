function [ x_best, y_best, z_best, rerr, it_cnt ] = Nav_hjs_3d_solve(eps, base_elements, x_prev, y_prev, z_prev,...
    max_iterations, precision_threshold, step_size)  

var_num = 3;
x0 = [ x_prev y_prev z_prev ];
f0 = eps(base_elements, x_prev, y_prev, z_prev);
deltas0 = [ step_size step_size step_size ]; 
deltas = deltas0;
it_cnt = 0;
...
while ((max(deltas) > precision_threshold) &&...
       ...
       while ((f1 < f0) && (it_cnt < max_iterations))
           ...
           [x2, f2, dts] = hjs_explore(x2, f2, deltas, var_num);                       
...
rerr = sqrt(f0);
    ...
    function [x_p, f_p, dlts2] = hjs_explore(x_c, f_c, dlts, v_num)
       ...
       for nn = 1:v_num
           ...
           f_p = eps(base_elements, x_p(1), x_p(2), x_p(3));           
